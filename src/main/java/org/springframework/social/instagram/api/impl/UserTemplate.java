...
public class UserTemplate extends AbstractInstagramOperations implements UserOperations {
	...
	public PagedMediaList getFeed(String maxId, String minId) {
		...
		if(maxId != null && !maxId.isEmpty()) params.put("max_id", maxId);
		if(minId != null && !minId.isEmpty()) params.put("min_id", minId);
	...
	public PagedMediaList getRecentMedia(String userId, int count, String maxId, String minId) {
		return getRecentMedia(userId, count, maxId, minId, 0, 0);
	}

	public PagedMediaList getRecentMedia(String userId, int count, String minId) {
		return getRecentMedia(userId, count, null, minId, 0, 0);
	}
	
	public PagedMediaList getRecentMedia(String userId, String maxId, String minId, long minTimestamp, long maxTimestamp) {
		return getRecentMedia(userId, 0, maxId, minId, minTimestamp, maxTimestamp);
	}
	
	public PagedMediaList getRecentMedia(String userId, int count, String maxId, String minId, long minTimestamp, long maxTimestamp) {
		Map<String,String> params = new HashMap<String, String>();
		if(count > 0) params.put("count", Integer.toString(count));
		if(minId != null && !minId.isEmpty()) params.put("min_id", minId);
		if(maxId != null && !maxId.isEmpty()) params.put("max_id", maxId);
