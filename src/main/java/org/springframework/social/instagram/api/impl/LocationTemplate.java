...
public class LocationTemplate extends AbstractInstagramOperations implements LocationOperations {
	...
	public PagedMediaList getRecentMedia(String locationId, String maxId, String minId, long minTimeStamp, long maxTimeStamp) {
		...
		if(minId != null && !maxId.isEmpty()) params.put("min_id", minId);
	...
	public List<Location> search(String foursquareId) {
		return get(buildUri(LOCATIONS_ENDPOINT + "search/", Collections.singletonMap("foursquare_id", foursquareId)), LocationList.class).getList();
	}	
