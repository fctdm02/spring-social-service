...
public class FlickrAdapter implements ApiAdapter<Flickr> {
    ...
    public UserProfile fetchUserProfile(Flickr api) {
    	Person profile = api.peopleOperations().getPersonProfile();
		return new UserProfileBuilder().setName(profile.getRealName()).setUsername(profile.getUserName()).build();
    }
    ...
    public void setConnectionValues(Flickr flickr, ConnectionValues values) {
		...
		values.setImageUrl(person.getProfileurl());
    }
